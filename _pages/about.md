---
permalink: /
title: ""
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

About Me
======

Howdy! I am an Economics PhD candidate at UC Santa Barbara, working under Doug Steigerwald (chair), Mitchell Hoffman (co-chair), Max Farrell, and Gonzalo Vasquez-Bare.

My research spans applied economics, econometrics, and causal machine learning. In my primary empirical work, I use modern causal inference tools to study programs and policies that shape entrepreneurial activity, financial access, and labor market outcomes. My job market paper, *Targeted Financial Access and Entrepreneurship*, examines how access to specialized non-bank lenders influences the rate and quality of new firm formation in the United States. On the methodological side, I draw on semi-parametric econometrics, debiased machine learning, and difference-in-differences to expand the empirical toolkit available to applied researchers. I also study how these insights can be used to improve the evaluation and interpretability of large language models through pairwise preference data and Bradley-Terry modeling.

Throughout my PhD, I have worked across public, private, and teaching roles. I spent a year as an economic research intern at the Federal Reserve Board of Governors and later worked in research and strategy for a Santa Barbara hedge fund focused on environmentally and socially responsible investments. More recently, I interned at Amazon in 2024 and 2025 as an economist with the Seller Fees Science and Fulfillment by Amazon Science teams, where I conducted policy-relevant causal analyses for e-commerce decisions. I have also served as the head teaching assistant for UCSB's Introduction to Econometrics course for four years.

Outside of work, my interests are broad. In spring and summer, I am an avid tennis player and an enthusiastic, if traitorous, pickleballer. In winter, you can find me snowboarding at Mammoth Mountain or hoop-netting for lobster in Long Beach. And if you share an appreciation for Italian food, art, and history, I would be glad to meet you.

You may reach me at my email, [spencersween@ucsb.edu]("spencersween@ucsb.edu"). I look forward to connecting. 

Recent Highlights
------
1. **[December 11, 2025]** A first draft of my job market paper is now available! You may find it [here]("https://spencersween.github.io/research"). I study how the Community Development Financial Institution program affects local entrepreneurship. Using a semi-parametric difference-in-differences design with debiased machine learning, I find CDFI lender entry into a local credit market increases startup formation, while measures of firm quality are unchanged. Feedback is warmly welcomed.
1. **[TBD]** On my [DML Projects]("https://spencersween.github.io/dmlprojects") page, I provide reproducible tutorials for applied economists on cutting-edge causal machine learning methods, including Automatic DML for complex functionals and Structural DML for heterogeneity-enriched model parameters.
1. **[TBD]** In a new [working paper]("https://spencersween.github.io/research"), I develop new debiased machine learning estimators for non-linear difference-in-differences and apply them to re-examine the impact of state tax credit incentives on startup activity and quality. A replication R implementation using DoubleML and Torch is available on my [DML Projects]("https://spencersween.github.io/dmlprojects") page, with R and Python package releases forthcoming soon.
1. **[TBD]** AI researchers, the applied economists are coming for you! AI researchers, the applied economists are coming for you. In a new project, I use structural debiased machine learning to estimate Bradley-Terry models for pairwise LLM comparison data, allowing preference parameters to vary with contextual information and conversation embeddings. A project overview is available [here]("https://spencersween.github.io/llmprojects"), with a working paper forthcoming.
